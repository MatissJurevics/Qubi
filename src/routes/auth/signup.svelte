...
<main class="h-screen w-screen flex flex-col justify-center items-center">
    <div class="min-w-[400px] min-h-[500px] bg-white/20 rounded-xl flex flex-col items-center">
        <h1 class="text-3xl text-content font-bold my-4">Sign up</h1>
        <div class="flex flex-col">
          ...
          <button class="btn btn-primary" on:click={() => console.log(email,password)}>Sign up</button>
        </div>
        <h3 class="text-center w-1/2 border-b border-white/50 my-6 text-xl opacity-80">or</h3>
        <div class="flex flex-row justify-center items-center w-full px-8 my-auto">
          <button class="btn btn-secondary" on:click={signIn}>Sign up with <span class="text-white ml-1">Google</span></button>
        </div>
